...
#!!! Should change to hardac, upload input
Input_filepath = "/Volumes/Genome_data/RRBS_H1/RRBS_H1_p25.bed"
Output_basepath = "/Volumes/Genome_data/RRBS_H1/Result/Result_"

Back_all = pd.DataFrame()
MetRatio_all = pd.DataFrame()

Input_format = ["chr", "start", "end", "covmeth", "methyl_ratio", "strand", "sequence"]


hg19_aligned = pd.read_table(Input_filepath, sep = "\t",
                         names = Input_format, index_col = False)

#print(type(hg19_aligned_pre["covmeth"]))
hg19_aligned["covmeth"] = hg19_aligned["covmeth"].str.strip("\'")

hg19_aligned[["meth", "coverage"]] = hg19_aligned["covmeth"].str.split("/", expand=True)

hg19_aligned = hg19_aligned.drop(['covmeth', 'methyl_ratio'], axis=1)

m = hg19_aligned.pop('meth')
hg19_aligned.insert(3, "methyl_ratio", m)

c = hg19_aligned.pop('coverage')
hg19_aligned.insert(3, "coverage", c)

#print(hg19_aligned)

hg19_aligned['methyl_ratio'] = hg19_aligned['methyl_ratio'].astype('float')
hg19_aligned['coverage'] = hg19_aligned['coverage'].astype('float')


hg19_aligned['methyl_ratio'] = hg19_aligned['methyl_ratio']/hg19_aligned['coverage']

hg19_aligned = hg19_aligned[hg19_aligned.methyl_ratio > 0]
...
for coverage_cutoff in range(0, 50, 5):
    ...
    for index, rows in hg19_positive.iterrows():
        ...
        if (float(rows['methyl_ratio']) <= 0.001): continue
